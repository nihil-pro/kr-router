...
export class AppRouter<T extends RouterConfig> extends EventTarget {
  ...
  constructor(config: T) {
    ...
    navigation.addEventListener("navigate", event => {
      ...
      if (event.sourceElement?.hasAttribute('target')) return;
      event.intercept(this);
    })
    ...
    Object.defineProperty(this, 'routes', { get: () => config });
    void this.handler();
  }


  private async handler() {
    // as [string, RouterRoutes<T>[keyof RouterRoutes<T>]][]
    ...
    this.dispatchEvent(new RouteNotFoundEvent(notFound));
  ...
  addEventListener(
    type: 'notfound',
    listener: (event: RouteNotFoundEvent) => void,
  ...
  addEventListener(
    ...
    // fixes late subscription, i.e. subscription race condition
    if (typeof listener === 'function') {
      if (type === 'notfound') {
        // @ts-ignore
        listener(new RouteNotFoundEvent(this.notFound));
      } else {
        // @ts-ignore
        listener(new RouteStateChangeEvent(type, this.routes[type]));
      }
    }
  }
